...
public class ModelCompte implements Serializable {
	...
	public List<Compte> getListeCompte() {
		if (liste == null) {
			liste = new ArrayList<>();
			// Récupérer uniquement les comptes non attribués à un parent
			List<DtoCompte> comptesNonAttribues = serviceCompte.listerTout();
			for (DtoCompte dto : comptesNonAttribues) {
				if(!dto.isFlagAdmin()) {
					liste.add(mapper.map(dto));  // Mapper Compte -> Compte si nécessaire
				}
			}
		}
		return liste;
	...
	@PostConstruct
	public void init() {
		getListeCompte(); // Initialiser la liste des comptes
	}
