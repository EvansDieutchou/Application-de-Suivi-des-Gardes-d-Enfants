...
public class DaoCompte implements IDaoCompte {
	...
	@TransactionAttribute(NOT_SUPPORTED)
	public List<Compte> listerComptesNonAttribues() {
	    // JPQL pour sélectionner les comptes qui ne sont pas associés à un parent
	    String jpql = "SELECT c FROM Compte c WHERE c.id NOT IN (SELECT p.compte.id FROM Parent p)";
	    var query = em.createQuery(jpql, Compte.class);
	    return query.getResultList();
	}
